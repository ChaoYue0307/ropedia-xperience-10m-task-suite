...
RUN_ID="${RUN_ID:-xperience10m_cosmos3_super_retrieval_task_probes_a100_20260619T000000Z}"
...
"$VENV_PY" "$MERGE_SCRIPT" \
  ...
  --output-dir "$OUT_DIR" \
  --tasks "$TASKS" \
  --shard-dirs $(for (( shard=0; shard<SHARDS; shard++ )); do printf ' results/omni_finetune/%s_shard%d' "$RUN_ID" "$shard"; done)
