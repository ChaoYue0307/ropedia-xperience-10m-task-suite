...
RUN_ID="${RUN_ID:-xperience10m_qwen3_omni_v6_future_task_probes_$(date -u +%Y%m%dT%H%M%SZ)}"
TASKS="${TASKS:-long_horizon_next_action,next_subtask_forecast,object_set_forecast}"
...
for ((offset = 0; offset < SHARDS; offset++)); do
  ...
  (
    ...
    "$VENV_PY" scripts/omni/eval_qwen3_omni_future_task_probes.py \
      "${COMMON_ARGS[@]}" \
      --run-id "$shard_run_id" \
...
if [[ "$failed" != "0" ]]; then
  echo "At least one future-task probe shard failed; inspect ${SHARD_DIRS[*]}" >&2
  echo "exit_code=1" >>"$OUT_DIR/launch_env.txt"
  exit 1
fi

"$VENV_PY" scripts/omni/merge_qwen3_omni_future_task_probe_shards.py \
  --run-id "$RUN_ID" \
  --output-dir "$OUT_DIR" \
...
echo "Qwen3 future task probes complete: $OUT_DIR"
