...
- cosmos3_super_training_readiness: runtime/GPU probe passes for Diffusers on 8 CUDA devices; camera-pose proxy action targets now cover 3,808/3,808 windows and pass the Cosmos3-Super action-LoRA contract audit; true fine-tuning still waits for the Cosmos3 batch packer and supervised action loss, so no Cosmos weights have been updated
...
| Cosmos 3 | First world-model/action-generation branch | Use now for future-window compatibility analysis and Cosmos3 action-LoRA trainer development; camera-pose proxy action targets are exported, but supervised Cosmos weights are still pending. |
