...
| Foundation-model plan | Current | `FOUNDATION_MODEL_PLAN.md`, `docs/data/foundation_model_plan.json` | Qwen3-Omni remains the first trainable held-out LoRA baseline; Cosmos 3 is added as the first world-model/action-generation branch; Cosmos3-Super now has camera-pose proxy action targets that pass the action-LoRA contract audit, while the trainer/loss and fine-tuned weights remain pending. OpenVLA/openpi/GR00T are policy candidates after robot-compatible action targets are explicit. |
| Cosmos3-Super action-target contract | Ready for trainer implementation | `scripts/omni/export_cosmos3_camera_pose_targets.py`, `results/omni_finetune/xperience10m_cosmos3_camera_pose_targets_20260608/target_manifest.json`, `results/omni_finetune/xperience10m_cosmos3_super_training_contract_audit_camera_pose_20260608/training_contract_audit.json` | The selected 128-episode JSONL is augmented with 3,808/3,808 valid `camera_pose` proxy `cosmos_action_target` records from SLAM pose deltas. This removes the action-target schema blocker for a Cosmos3-Super action-LoRA smoke run, but does not update Cosmos weights; the remaining work is a Cosmos3 batch packer, supervised action loss, and one-episode overfit. |
...
- Foundation-model selection is now explicit: Qwen3-Omni is the immediate
  ...
  has a camera-pose proxy action-target contract ready for trainer
